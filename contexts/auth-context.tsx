...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const checkSession = async () => {
    try {
      ...
      console.log(`Verificando sesión (intento ${checkCount + 1})...`)
      ...
      const checkPromise = async () => {
        ...
        // Estrategia 4: Obtener el usuario directamente de la API
        try {
          console.log("Intentando obtener usuario directamente de la API")
          const response = await fetch("/api/auth/me")
          if (response.ok) {
            const apiUser = await response.json()
            console.log("Usuario obtenido de la API:", apiUser)
            setDebugInfo((prev: any) => ({ ...prev, apiUser }))
            if (apiUser && apiUser.id) {
              setUser(apiUser as User)
              return
            }
          } else {
            console.log("Error al obtener usuario de la API:", response.statusText)
            setDebugInfo((prev: any) => ({ ...prev, apiError: response.statusText }))
          }
        } catch (apiError) {
          console.error("Error al llamar a la API:", apiError)
          setDebugInfo((prev: any) => ({ ...prev, apiError }))
        }

        // Estrategia 5: Crear un nuevo usuario si no se encuentra por ningún método
